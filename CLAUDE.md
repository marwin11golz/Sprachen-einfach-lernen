...
- **The Fehlerkartei (error deck) admits on `wrong > 0` but releases on a recovery streak, not on `wrong` itself.** `wrong` never decrements by design (see `repetitions` below — progress counters don't shrink), so membership can't hang off `wrong > 0` alone: the deck would only ever grow, no matter how many times a card was answered correctly since. `isDifficult()` in `App.jsx` also requires `earlyStep < FEHLERKARTEI_ERHOLT` (3) — `earlyStep` already counts consecutive successful ratings since the last "Nochmal", reset to 0 on the next one (see the ladder below), so a second field for the same thing was unnecessary. A card leaves after three recoveries in a row (hard counts as a recovery too — it's still successful recall) and returns the moment it lapses again.
