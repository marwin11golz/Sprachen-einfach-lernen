...
- **`src/lib/fsrs.js`** — the FSRS-6 scheduling core (stability/difficulty/retrievability/interval formulas, standard weights) plus the target-retention constants (`RETENTION_DEFAULT`/`MIN`/`MAX`, `clampRetention`). Pure functions, no imports.
...
- **`src/hooks/useVocabStore.js`** — owns `cards`, activity, `flipped`, `newCardsPerDay`, `retention`, persistence. Exposes **only intent-based actions** (`addCards`, `rateCard`, `deleteCard`, `importData`), never a raw `setCards` — that is what guarantees every mutation stamps `updatedAt`.
...
- **Preferences sync carries only `flipped`.** `newCardsPerDay` and `retention` are deliberately local-only: they're pacing preferences, not progress, and keeping them out means zero changes to `sync.js`. `retention` still reaches the cloud *indirectly* — changing it reschedules the cards, and those carry `updatedAt` like any other edit.
...
- **Target retention is the one knob over review density**, exposed in the dashboard as named steps (`DICHTE_STUFEN` in `App.jsx`) and defaulting to `0.95` — deliberately above Anki's `0.9`, under which a card rated "Gut" runs 2 → 11 → 46 → 163 days and everything already learned disappears for months. Since the fixed ladder above, it only governs what happens **after** step 7 — `rescheduleCard()` returns cards still on the ladder untouched, and the reference sequences in the comment over `DICHTE_STUFEN` are therefore post-ladder ones, not sequences from day one. Changing it also **reschedules the existing cards** (`rescheduleCard()` → `useVocabStore.setRetention`), otherwise the knob would do nothing for anything already learned. Two rules there, and they differ on purpose: the one-off migration on load (for users who predate the knob) rewrites due dates **without** stamping `updatedAt` — the calculation is deterministic, so every device reaches the same result on its own, and stamping could let the device opened first overwrite the other's unsynced ratings. An explicit knob change **does** stamp, because the setting is local-only and the other device cannot derive it.
- **No interval number appears anywhere in the UI.** Scheduling is background work: the knob shows named steps only, the rating buttons carry no day counts, and the ladder's sequences live in a comment over `DICHTE_STUFEN`, not on screen. Someone who reads that a card returns in 272 days starts rating the number instead of their own recall. The card list's per-card `fällig <date>` is not an exception — a due date says when *this* card is next up, not what a rating hands out.
